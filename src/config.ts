...
export function getProxyHostPort(): string {
  ...
  if (!proxyHostPort) {
    throw new Error(`mockserver needs the PROXY_HOST_PORT environment variable to be set.
...
E.g. PROXY_HOST_PORT='my-service.com:2342'`);
  ...
  if (!proxyHostPort.includes(":")) {
    log.warn("Your PROXY_HOST_PORT has no port configured, will default to 80");
